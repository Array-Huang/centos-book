# Summary

* [封面](README.md)
* [第一章：CentOS的系统服务管理系统](content/systemd.md)
* [第二章：CentOS的文件系统](content/file-system.md)
* [第三章：CentOS的进程系统](content/process.md)
* [第四章：CentOS的系统用户与用户组管理](content/auth.md)
* [第五章：Vim的使用](content/vim.md)
* [第六章：安装RPM包或源码包](content/install_rpm_and_source.md)
* [第七章：shell相关](content/shell.md)
* [第八章：神奇的xargs命令](content/xargs.md)
* [第九章：CentOS的历史命令](content/history.md)
* [第十章：监控CentOS的系统状态](content/monitor.md)
* [第十一章：SSH远程登录](content/ssh.md)
* [第十二章：备份工具rsync](content/rsync.md)